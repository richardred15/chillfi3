...
class ThemeSwitcher {
    ...
    updateSVGGradients() {
        ...
        const primary = computedStyle.getPropertyValue('--accent-primary').trim();
        const secondary = computedStyle.getPropertyValue('--accent-secondary').trim();
        
        // Update all gradient stops in inline SVGs
        const gradientStops = document.querySelectorAll('svg defs linearGradient stop');
        gradientStops.forEach(stop => {
            if (stop.getAttribute('offset') === '0' || !stop.getAttribute('offset')) {
        ...
        const gradientIds = ['logoGradient', 'playGradient', 'playStrokeGradient', 'homeGradient', 'searchGradient', 'menuGradient'];
    ...
    async updateExternalSVGs(primary, secondary) {
        ...
        allSvgImages.forEach(img => {
            if (img.src.includes('logo.svg') || img.src.includes('menu.svg') || 
                img.src.includes('home.svg') || img.src.includes('search.svg') || 
                img.src.includes('library.svg') || img.src.includes('playlist.svg') ||
                img.src.includes('upload.svg') || img.src.includes('queue.svg') ||
                img.src.includes('volume.svg') || img.src.includes('activity.svg')) {
        ...
        const themedSvgImages = document.querySelectorAll('img[data-theme-svg="true"]');
    ...
    createThemeSelector() {
        ...
        Object.entries(this.themes).forEach(([key, name]) => {
            ...
            themeItem.className = `user-dropdown-item theme-item ${key === this.currentTheme ? 'active' : ''}`;
            themeItem.innerHTML = `
    ...
    updateDynamicContent() {
        ...
        const elementsToUpdate = document.querySelectorAll('.no-artwork, .card-image:empty');
