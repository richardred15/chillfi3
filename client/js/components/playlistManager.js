...
export default class PlaylistManager {
    ...
    init() {
        //this.loadPlaylists();
        this.api.events.on("authenticated", () => {
            this.loadPlaylists();
        });
    }
    ...
    updateSidebar() {
        ...
        const existingItems = playlistSection.querySelectorAll(
            ".nav-item, .playlist-empty"
        );
        ...
        if (this.playlists.length === 0) {
            ...
            if (
                ...
                typeof window.themeSwitcher.updateThemeIcons === "function"
            ) {
    ...
    showPlaylistContent(playlist, songs) {
        ...
        const playlistNavItem = Array.from(navItems).find(
            (item) =>
                item.querySelector(".nav-item-text")?.textContent ===
                playlist.name
        );
        if (playlistNavItem) {
        ...
        playlistSection.innerHTML = `
            <div class="section-header">
                ...
                <div class="section-actions">
                    ${editButtons}
                    <button class="btn btn-primary" onclick="window.playlistManager.playPlaylist(${
                        playlist.id
                    })">Play All</button>
                </div>
            </div>
        ...
        songs.forEach((song, index) => {
            ...
            songItem.innerHTML = `
                ...
                <div class="song-duration">${this.formatDuration(
                    song.duration || 0
                )}</div>
            `;
    ...
    showEditPlaylistModal(playlist, songs) {
        ...
        document.getElementById("editPlaylistPublic").checked =
            playlist.is_public;
        ...
        if (songs.length === 0) {
            songsContainer.innerHTML =
                '<p style="color: var(--text-secondary); font-style: italic;">No songs in this playlist</p>';
        } else {
            songs.forEach((song) => {
                const songItem = document.createElement("div");
                ...
                songItem.innerHTML = `
                    <label class="edit-song-label">
                        <div class="edit-song-info">
                            ...
                            <div class="edit-song-artist">${
                                song.artist || "Unknown Artist"
                            }</div>
                        </div>
                        <input type="checkbox" class="edit-song-checkbox" data-song-id="${
                            song.id
                        }" checked>
                    </label>
                `;
        ...
        document.getElementById("editPlaylistSave").onclick = async () => {
            const name = document
                .getElementById("editPlaylistName")
                .value.trim();
            const isPublic =
                document.getElementById("editPlaylistPublic").checked;
            ...
            try {
                ...
                await this.api.updatePlaylist(playlist.id, {
                    name,
                    is_public: isPublic,
                });
