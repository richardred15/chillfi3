...
export default class AlbumView {
                        ...
                        <div class="album-details">
                            ...
                            <div class="album-actions">
                                <button class="album-play-all">
                                    <img src="client/icons/play_white.svg" alt="Play" width="20" height="20">
                                    Play All
                                </button>
                            </div>
                        </div>
        ...
        try {
            ...
            if (window.player) {
                ...
                for (const albumSong of this.currentAlbum.songs) {
                    try {
                        const songResponse = await this.api.playSong(
                            albumSong.id
                        );
                        queue.push({
    ...
    updatePageMetadata() {
        ...
        this.setMetaTag(
            "description",
            `Listen to ${title} by ${artist} on ChillFi3. Album contains ${trackCount} tracks${
                year ? ` from ${year}` : ""
            }.`
        );
