                ...
                <form class="metadata-form" id="metadataForm">
                    <div class="form-group">
                        ...
                        <div class="album-art-section">
                            ...
                            <button type="button" class="btn-upload-art" onclick="document.getElementById('albumArtInput').click()">Change ${
                                isArtist ? "Image" : "Art"
                            }</button>
                        </div>
                    </div>
                    ${
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="artistName" name="name" value="${
                                this.currentSong.title || ""
                            }" required>
                        </div>
                        <div class="form-group">
                            <label for="artistBio">Biography</label>
                            <textarea id="artistBio" name="bio" rows="4" placeholder="Enter artist biography...">${
                                this.currentSong.bio || ""
                            }</textarea>
                        </div>
                    `
                            : `
                        <div class="form-group" ${
                            this.isAlbum ? 'style="display: none;"' : ""
                        }>
    ...
    async handleSubmit(e, modal) {
        ...
        try {
            ...
            if (this.editMode === "artist") {
                ...
                response = await this.api.updateArtist(
                    this.currentSong.id,
                    artistUpdates
                );
                ...
                response = await this.api.updateAlbum(
                    this.currentSong.album_id,
                    albumUpdates
                );
            } else {
                // Use song update endpoint
                response = await this.api.updateSong(
                    this.currentSong.id,
                    metadata
                );
            }
            ...
            if (response.success) {
                ...
                if (window.contentManager) {
                    window.shareManager.handleShareURL();
                }
            } else {
