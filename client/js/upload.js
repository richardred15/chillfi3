...
class UploadManager {
    ...
    setupUploadModal() {
        ...
        const progressCancelButton = document.getElementById('cancelCurrentUploadButton');
        ...
        if (dropzone) {
            ...
            newDropzone.addEventListener('drop', async (e) => {
                ...
                e.stopPropagation();
                newDropzone.classList.remove('dragover');
                const allFiles = await window.getAllFilesFromDrop(e.dataTransfer);
    ...
    addFileToUI(file) {
        ...
        console.log('File item added to DOM, total items:', uploadFiles.children.length);
    ...
    extractFileMetadata(file, fileItem) {
        ...
        reader.onload = (e) => {
            ...
            if (!this.id3Worker) {
                ...
                this.id3Worker.onmessage = (event) => this.handleWorkerMessage(event);
            }
    ...
    handleWorkerMessage(event) {
        ...
        if (success && tags) {
            ...
            if (titleInput) titleInput.value = tags.title || fileItem._fileObject.name.replace(/\.[^/.]+$/, '');
            if (artistInput) artistInput.value = tags.artist || 'Unknown Artist';
            if (albumInput) albumInput.value = tags.album || 'Unknown Album';
            if (genreInput) genreInput.value = tags.genre || 'Electronic';
            if (yearInput) yearInput.value = tags.year || new Date().getFullYear();
    ...
    async extractMetadataFromItem(item) {
        ...
        return {
            ...
            artist: item.querySelector('.file-artist')?.value || 'Unknown Artist',
            ...
            trackNumber: parseInt(item.querySelector('.file-track')?.value) || null,
            duration: this.parseDuration(item.querySelector('.file-duration')?.value),
            artwork: artwork
        };
    }
        ...
        for (let i = 0; i < this.uploadQueue.length; i++) {
            ...
            try {
                ...
                if (this.isNetworkError(error)) {
                    this.failedUploads.push({ file, metadata, element, index: i });
                    element.classList.add('paused');
                    this.showFileError(element, 'Upload paused - will resume when connection restored');
                } else {
                    element.classList.add('failed');
    ...
    isNetworkError(error) {
        const networkErrors = ['Failed to fetch', 'Network request failed', 'Upload failed', 'ERR_NETWORK', 'Bad Gateway', '502'];
    ...
    async resumeFailedUploads() {
        ...
        for (const upload of uploadsToRetry) {
            ...
            try {
                ...
                this.updateUploadProgress(index + 1, this.uploadQueue.length, file.name, 0);
                
                const result = await this.uploadFileWithProgress(file, metadata, localStorage.getItem('chillfi_token'), (progress) => {
                    this.updateUploadProgress(index + 1, this.uploadQueue.length, file.name, progress);
                });
                ...
                if (result.results[0].success) {
                    ...
                    const errorDiv = element.querySelector('.file-error-message');
    ...
    finishUpload() {
        ...
        // Count results
        const uploaded = document.querySelectorAll('.file-item.uploaded').length;
    ...
    showFileError(fileItem, errorMessage) {
        ...
        fileItem.style.border = isPaused ? '2px solid #ffa500' : '2px solid #ff4444';
        ...
        if (!errorDiv) {
            ...
            errorDiv.style.cssText = `color: ${isPaused ? '#ffa500' : '#ff4444'}; font-size: 12px; margin-top: 8px; padding: 4px;`;
            fileItem.appendChild(errorDiv);
        }
        errorDiv.textContent = isPaused ? errorMessage : `Error: ${errorMessage}`;
    }
    ...
    clearAll() {
        if (!confirm('Are you sure you want to clear all uploads? This will cancel any uploads in progress.')) {
            return;
        }
