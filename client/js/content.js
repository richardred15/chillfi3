...
class ContentManager {
    ...
    async loadAllSongs() {
        try {
            ...
            const response = await this.api.getSongs({}, 1, this.HOME_SECTION_ITEM_COUNT);
    ...
    showEmptyState() {
        ...
        if (container) {
            container.innerHTML = '<div class="empty-state">No songs uploaded yet. Upload some music to get started!</div>';
        }
    }
    ...
    async loadRecentlyPlayed() {
        try {
            const response = await this.api.getRecentlyPlayed(this.HOME_SECTION_ITEM_COUNT);
            const songs = response.data?.items || response.songs || [];
    ...
    renderSongList(songs) {
        ...
        songs.forEach((song, index) => {
            ...
            const sanitizedTitle = this.sanitizeHTML(song.title || 'Unknown Title');
            const sanitizedArtist = this.sanitizeHTML(song.artist || 'Unknown Artist');
    ...
    async loadAlbums() {
        try {
            const response = await this.api.getAlbums(1, this.HOME_SECTION_ITEM_COUNT);
            ...
            if (container) {
                container.innerHTML = '<div class="empty-state">No albums available</div>';
            }
        }
    }
    ...
    renderAlbumGrid(albums) {
        ...
        albums.forEach(album => {
            ...
            const sanitizedTitle = this.sanitizeHTML(album.title || 'Unknown Album');
            const sanitizedArtist = this.sanitizeHTML(album.artist || 'Unknown Artist');
    ...
    showSearchResults(query, songs, page = 1, limit = 20, total = 0) {
        ...
        if (queryDisplay) {
            queryDisplay.textContent = query ? `"${this.sanitizeHTML(query)}"` : 'Type to search...';
        }
        ...
        // Render pagination
        this.renderPagination(page, limit, total, 'search', 'searchPaginationNav', 'searchPageSizeSelect', query);
        
        // Set active nav item for search
        const navItems = document.querySelectorAll('.nav-item');
        navItems.forEach(item => item.classList.remove('active'));
        const searchNavItem = Array.from(navItems).find(
    ...
    renderSearchResults(songs) {
        ...
        songs.forEach((song, index) => {
            ...
            // Add click handler for play
            songItem.addEventListener('click', () => this.playSong(song, 'search'));
    ...
    async showMyLibrary(page = 1, limit = 24) {
        try {
            const artists = await this.getUserArtists('current_user', page, limit);
            this.showArtistsSection('My Library', artists, 'library', page, limit);
    ...
    async showUserLibrary(username, page = 1, limit = 24) {
        try {
            ...
            const title = username === this.api.user?.username ? 'My Library' : `${username}'s Library`;
            this.showArtistsSection(title, artists, 'user_library', page, limit);
    ...
    async getUserArtists(username, page = 1, limit = 24) {
        const response = await this.api.getSongs({ uploadedBy: username }, 1, 1000); // Get all songs
        const songs = response.data?.items || response.songs || [];
    ...
    showArtistsSection(title, artistsData, context, page, limit) {
        ...
        artistsData.artists.forEach(artist => {
            ...
            card.innerHTML = `
                <div class="card-image" style="${artworkStyle}"></div>
                <div class="card-content">
                    <div class="card-title">${this.sanitizeHTML(artist.name)}</div>
                    <div class="card-subtitle">${artist.albumCount} albums • ${artist.songCount} songs</div>
                </div>
                <button class="card-play">
                    <img src="client/icons/play.svg" alt="Play" width="24" height="24">
                </button>
            ...
            card.addEventListener('click', () => {
                const username = context.includes('user_library') ? context.split('_')[1] + '_library' : 'current_user';
        ...
        // Setup pagination
        this.renderPagination(page, limit, artistsData.total, context, navId, selectId);
    }
    
    // Show albums for a specific artist
    async showArtistAlbums(artistName, username = 'current_user') {
        try {
            const response = await this.api.getSongs({ uploadedBy: username, artist: artistName }, 1, 1000);
            const songs = response.data?.items || response.songs || [];
    ...
    showAlbumsForArtist(artistName, albums, username) {
        ...
        section.style.display = 'block';
        section.querySelector('.section-title').textContent = `${artistName} - Albums`;
        ...
        if (backButton) {
            ...
            if (username === 'current_user' || username.includes('user_library')) {
        ...
        albums.forEach(album => {
            ...
            card.innerHTML = `
                <div class="card-image" style="${artworkStyle}"></div>
                <div class="card-content">
                    <div class="card-title">${this.sanitizeHTML(album.title)}</div>
                    <div class="card-subtitle">${album.year || 'Unknown Year'} • ${album.songs.length} songs</div>
                </div>
                <button class="card-play">
                    <img src="client/icons/play.svg" alt="Play" width="24" height="24">
                </button>
            ...
            card.addEventListener('click', () => {
                if (window.albumView) {
                    ...
                    URLManager.setURL({ album: album.title, artist: album.artist });
                }
            });
    ...
    setupSeeAllHandlers() {
        ...
        const recentlyPlayedSeeAll = document.querySelector('#recentlyPlayedSection .section-action');
        ...
        // Top Albums See All
        const topAlbumsSeeAll = document.querySelector('#topAlbumsSection .section-action');
        if (topAlbumsSeeAll) {
    ...
    async showAllRecentlyPlayed(page = 1, limit = 20) {
        try {
            ...
            const total = response.total || response.data?.pagination?.total || songs.length;
            
            this.showPaginatedSectionWithControls('All Recently Played', songs, 'recently_played', page, limit, total);
        } catch (error) {
    ...
    showPaginatedSectionWithControls(title, songs, context, page, limit, total) {
        const sectionId = context === 'library' ? 'librarySection' : 
                         context === 'recently_played' ? 'allRecentlyPlayedSection' : 'paginatedSection';
        const listId = context === 'library' ? 'libraryList' : 
                      context === 'recently_played' ? 'allRecentlyPlayedList' : 'paginatedList';
        const navId = context === 'library' ? 'libraryPaginationNav' : 
                     context === 'recently_played' ? 'allRecentlyPlayedPaginationNav' : 'paginationNav';
        const selectId = context === 'library' ? 'libraryPageSizeSelect' : 
                        context === 'recently_played' ? 'allRecentlyPlayedPageSizeSelect' : 'pageSizeSelect';
        ...
        if (pageSizeSelect) {
            ...
            pageSizeSelect.onchange = () => {
                if (context === 'library') {
                    ...
                    this.showAllRecentlyPlayed(1, parseInt(pageSizeSelect.value));
                }
            };
        }
        ...
        if (container) {
            ...
            songs.forEach((song, index) => {
                ...
                songItem.innerHTML = `
                    ...
                    <div class="song-artist-desktop">${song.artist || 'Unknown Artist'}</div>
                    <div class="song-duration">${formatDuration(song.duration)}</div>
                `;

                songItem.addEventListener('click', () => this.playSong(song, context));
                container.appendChild(songItem);
            });
        }
    ...
    renderPagination(currentPage, limit, total, context, navId, selectId, searchQuery = null) {
        ...
        paginationHTML += `<button class="pagination-btn${prevDisabled ? ' disabled' : ''}" data-page="${currentPage - 1}" ${prevDisabled ? 'disabled' : ''}>Previous</button>`;
        ...
        paginationHTML += `<button class="pagination-btn${nextDisabled ? ' disabled' : ''}" data-page="${currentPage + 1}" ${nextDisabled ? 'disabled' : ''}>Next</button>`;
        ...
        container.querySelectorAll('.pagination-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (isNaN(page) || page < 1 || isNaN(limit) || limit < 1) return;
    ...
    renderAlbumPagination(currentPage, limit, total) {
        this.renderPagination(currentPage, limit, total, 'albums', 'albumsPaginationNav', 'albumPageSizeSelect');
    }
    ...
    showHomeSections() {
        const homeSectionIds = [
            'recentlyPlayedSection', 'topAlbumsSection', 'recentlyAddedSection'
        ];
    ...
    async playSong(song, context = 'recent') {
        ...
        const wrappedApiCall = errorHandler ? 
            errorHandler.wrapAsync(this.api.playSong.bind(this.api), 'Play song') : 
    ...
    showSharedPlaylist(playlist, songs, shareManager) {
        ...
        const canEdit = this.api && this.api.user && (this.api.user.id === playlist.user_id || this.api.user.is_admin);
        ...
        playlistSection.innerHTML = `
            <div class="section-header">
                <h2 class="section-title">${this.sanitizeHTML(playlist.name)}</h2>
                <div class="section-actions">
                    ${editButtons}
                    <button class="btn btn-primary" onclick="window.shareManager.playSharedPlaylist(${playlist.id})">Play All</button>
                </div>
            </div>
            <div class="playlist-info">
                <p>${songs.length} songs • ${playlist.is_public ? 'Public' : 'Private'} • by ${this.sanitizeHTML(playlist.owner_username || 'Unknown')}</p>
            </div>
            <div class="song-list">
        ...
        if (songs.length === 0) {
            ...
            songs.forEach((song, index) => {
                ...
                songItem.innerHTML = `
                    ...
                    <div class="song-artist">${this.sanitizeHTML(song.artist || 'Unknown Artist')}</div>
        ...
        if (type === 'profile') {
            ...
            sharedSection.innerHTML = `
                ...
                <div class="shared-item">
                    <div class="shared-avatar${!avatarUrl ? ' no-image' : ''}" ${avatarUrl ? `style="background-image: url(${avatarUrl});"` : ''}>
                        ${!avatarUrl ? this.sanitizeHTML((item.display_name || item.username).charAt(0).toUpperCase()) : ''}
                    </div>
                    <div class="shared-info">
                        <h3>${this.sanitizeHTML(item.display_name || item.username)}</h3>
                        <p>${this.sanitizeHTML(item.bio || 'No bio available')}</p>
                        <div class="shared-stats">
                            <div><strong>${items.length}</strong> <span>Recent Uploads</span></div>
                        </div>
                    </div>
                </div>
                ${items.length > 0 ? `
            ...
            if (items.length > 0) {
                ...
                items.forEach((song, index) => {
                    ...
                    songItem.innerHTML = `
                        ...
                        <div class="song-artist">${this.sanitizeHTML(song.artist || 'Unknown Artist')}</div>
                    `;
                    songItem.addEventListener('click', () => shareManager.playSong(song.id));
                    songsList.appendChild(songItem);
                });
            }
        }
